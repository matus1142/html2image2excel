...
def process_html_content(html_content, filename):
    ...
    try:
        ...
        # Create Excel in memory with optimization
        output = io.BytesIO()
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if text_content:
                text_df = pd.DataFrame(text_content, columns=['Element_Type', 'Content'])
                text_df.to_excel(writer, sheet_name='Text_Content', index=False)
                del text_df
            
            # Process meta information
            if meta_info:
                meta_df = pd.DataFrame(meta_info, columns=['Meta_Tag', 'Content'])
            ...
            summary_data = [
                ...
                ['File_Size_KB', round(len(html_content.encode('utf-8')) / 1024, 2)],
                ['Tables_Found', len(tables)],
                ['Text_Elements_Found', len(text_content)],
                ['Meta_Tags_Found', len(meta_info)],
